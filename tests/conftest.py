...
def example_submodel_collection() -> SubmodelElementCollection:
    return Version(id_short="version_info", version="1.2.2", product_type="type1")
...
@pytest.fixture(scope="function")
def example_submodel_with_id_reference_components() -> (
...
def example_submodel_with_product_association(
    example_aas_comp1: ProductAas, example_aas_comp2: ProductAas
...
def example_aas_comp1() -> AAS:
    return ProductAas(
        id_short="comp1",
        example_submodel=SubmodelBom(
            id_short="bom_comp1", components=[], num_components=0
        ),
        info=ProductInfo(
            ...
            manufacturer="manufacturer1",
            product_version=Version(
                id_short="version_info_comp1", version="1.2.2", product_type="type1"
            ),
        ),
    )
...
def example_aas_comp2() -> AAS:
    return ProductAas(
        id_short="comp2",
        example_submodel=SubmodelBom(
            id_short="bom_comp2", components=[], num_components=0
        ),
        info=ProductInfo(
            ...
            manufacturer="manufacturer1",
            product_version=Version(
                id_short="version_info_comp2", version="1.2.2", product_type="type1"
            ),
        ),
    )
...
def example_submodel_bom() -> SubmodelBom:
    return SubmodelBom(
        id="example_submodel_bom_id", components=["comp1", "comp2"], num_components=2
    )


@pytest.fixture(scope="function")
def example_basemodel_bom_with_id() -> BaseModelBomWithId:
    return BaseModelBomWithId(
        id="example_basemodel_bom_with_id",
        components=["comp1", "comp2"],
        num_components=2,
    )
...
def example_object_bom_with_id() -> ObjectBomWithId:
    return ObjectBomWithId(
        id="example_object_bom_with_id", components=["comp1", "comp2"], num_components=2
    )


@pytest.fixture(scope="function")
def example_basemodel_bom_with_identifier_attribute() -> (
    BaseModelBomWithIdentifierAttribute
):
    return BaseModelBomWithIdentifierAttribute(
        other_name_id_attribute="example_basemodel_bom_with_identifier_attribute_id",
        components=["comp1", "comp2"],
        num_components=2,
...
def example_object_with_identifier_attribute() -> ObjectBomWithIdentifierAttribute:
    return ObjectBomWithIdentifierAttribute(
